...
export default function normalizedInteractionEvents(element) {
  ...
  let mouseY
  let mods = {}
  let changeListener = mouseChange(element, function(pbuttons, px, py, pmods) {
    mouseX = px
    mouseY = py
    buttons = pbuttons
    mods = pmods
  ...
  function onTouchStart(event) {
    ...
    for (let i = 0; i < event.changedTouches.length; i++) {
      ...
      if (idx === -1 && activeTouchCount < 2) {
        // var first = activeTouchCount === 0;x

        // newest and previous finger (previous may be undefined)
        let newIndex = fingers[0] ? 1 : 0
        // let oldIndex = fingers[0] ? 0 : 1;
        var newFinger = {
          position: [0, 0],
          touch: null
        }
        ...
        // let oldTouch = fingers[oldIndex] ? fingers[oldIndex].touch : undefined;
      }
    }
  ...
  function onTouchRemoved(event) {
    ...
    for (let i = 0; i < event.changedTouches.length; i++) {
      ...
      if (idx !== -1) {
        ...
        // let otherTouch = fingers[otherIdx]
