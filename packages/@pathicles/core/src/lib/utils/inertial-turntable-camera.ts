...
import vec3RotateY from 'gl-vec3/rotateY'
import vec3RotateX from 'gl-vec3/rotateX'
import vec3Equals from 'gl-vec3/equals'
import vec3Add from 'gl-vec3/add'
import vec3ScaleAndAdd from 'gl-vec3/scaleAndAdd'
import vec3Copy from 'gl-vec3/copy'
...
import mat4Scale from 'gl-mat4/scale'
...
const MAX_PHI = Infinity //Math.PI * 0.5 - 1e-4
const MIN_PHI = -Infinity //Math.PI * 0.5 + 1e-4
...
export default function createCamera(opts) {
  ...
  function decay(dt) {
    const panDecay = params.panDecayTime
      ? Math.exp(-dt / params.panDecayTime / Math.LN2)
      : 0
    const zoomDecay = params.zoomDecayTime
      ? Math.exp(-dt / params.zoomDecayTime / Math.LN2)
      : 0
    const rotateDecay = params.rotationDecayTime
