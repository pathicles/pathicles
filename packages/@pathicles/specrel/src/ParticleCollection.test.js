...
test('ParticleCollection default constructor ', () => {
  ...
  expect(pCollection.particles.length).toBe(0);
...
test('ParticleCollection.create({particleCount: 1})', () => {
  ...
  expect(pCollection.particles.length).toBe(1);
...
test('new ParticleCollection({count: 2, particleTypeDistribution: [ParticleTypes.ELECTRON]})', () => {
  ...
  expect(pCollection._particles.length)
    .toBe(2)

  expect(pCollection._particles.map(p => p.particleType))
    .toEqual([ParticleTypes.ELECTRON, ParticleTypes.ELECTRON])
})
...
test('new ParticleCollection({particleCount: 3, particleTypeDistribution: "PROTON"})', () => {
  ...
  expect(pCollection._particles.map(p => p.particleType))
    .toEqual([ParticleTypes.PROTON, ParticleTypes.PROTON, ParticleTypes.PROTON])
...
test('new ParticleCollection({particleCount: 7, particleTypeDistribution: "multiple"})', () => {
  ...
  const pCollection = ParticleCollection.create(
    {
      particleCount: 7,
      particleTypeDistribution: [ParticleTypes.PROTON, ParticleTypes.PHOTON, ParticleTypes.ELECTRON, ParticleTypes.ELECTRON]
    });
  ...
  expect(pCollection._particles.map(p => p.particleType))
    .toEqual([ParticleTypes.PROTON, ParticleTypes.PHOTON, ParticleTypes.ELECTRON, ParticleTypes.ELECTRON, ParticleTypes.PROTON, ParticleTypes.PHOTON, ParticleTypes.ELECTRON])

  const pCollectionClone = pCollection.clone()
  expect(pCollectionClone._particles.length)
    .toBe(7);

  expect(pCollectionClone._particles.map(p => p.particleType))
    .toEqual([ParticleTypes.PROTON, ParticleTypes.PHOTON, ParticleTypes.ELECTRON, ParticleTypes.ELECTRON, ParticleTypes.PROTON, ParticleTypes.PHOTON, ParticleTypes.ELECTRON])
...
test('new ParticleCollection({particleCount: 5, particleTypeDistribution: "multiple"})', () => {
  ...
  expect(pCollection._particles.map(p => p.particleType))
    .toEqual([ParticleTypes.PROTON, ParticleTypes.PHOTON, ParticleTypes.ELECTRON, ParticleTypes.ELECTRON, ParticleTypes.PROTON])

  pCollection.distributeLocation({bunchShape: "ROW", dx: 0.1})
  expect(pCollection.particles.map(p => arrayToNumbers(p.position)))
...
describe('new ParticleCollection({particleCount: 251, particleTypeDistribution: "multiple"}) square distribution', () => {
  ...
  it("has correct number of particles", () => {
    ...
    expect(pCollection._particles.length).toBe(251);
  ...
  it("has correctly distributed particle positions", () => {
    ...
    // 251 distinct values
    expect(positions.length).toEqual(positionSet.size)
  ...
  it("has correctly distributed particle momenta", () => {
    ...
    const momenta = pCollection.particles.map(p => [p.momentum[0].toNumber(), p.momentum[1].toNumber(), p.momentum[2].toNumber()])
  ...
  pCollection.setMomentaFromKineticEnergy({kineticEnergy__eV: 1000, direction: [1, 0, 0]})
